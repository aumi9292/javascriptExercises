//given a string, parse and return two pieces of information--the number of email messages in the string and the range of dates of emails in the string 

//rules
//emails are separated by ##||##
//Each email has 5 parts (sender, subj, date, recip, body) delimited by #/#

//input: string
//out: string: Count of email: 5
//Date Range: Sat Jun 25 2016 0 Thu Aug 11 2016

//algorithm
// split the long string by email delimiter #||# [email1, email2, email3]
// save the length of this array as emailCount 3
// map over the array of different emails just the date [date1, date2, date3] 
// sort dates, return the min and max dates to be entered into string (use Date object methods? new Date (dateFromString))
// format the dates to be Day Mon 14 yyyy
...
function parseDates(emailData) {
  let emails = emailData.split('#||#');
  let mapped = emails.map(email => {
    let dateStr = email.split(/#\/#/)[2].split(' ')[1];
    return new Date(dateStr);
  });
  return mapped.sort((a, b) => a - b);
};

function getMinMaxFormattedDates(emailData) {
  let dates = parseDates(emailData);
  let minMaxCount = {
...
function formatCountAndDateRange(emailData) {
  let {min, max, count} = getMinMaxFormattedDates(emailData);
  let countStr = `Count of emails: ${count}`;
  let rangeStr = `Date Range: ${min} - ${max}`;
  return [countStr, rangeStr];
}

function mailCount(emailData) {
  console.log(formatCountAndDateRange(emailData).join('\n'));
}
