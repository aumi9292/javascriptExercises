...
function determineSentiment(text) {
  ...
  let count = text.toLowerCase().split(' ').reduce((total, word) => {
  ...
  return {pos: positives, neg: negatives, count: count};
}

function sentiment(text) {
  const {pos, neg, count} = determineSentiment(text);
  let sent = count > 0 ? 'positive' : 'negative';
  console.log(`There are ${pos.length} positive words in the text:`);
  pos.forEach(word => console.log(word));
  console.log(`There are ${neg.length} negative words in the text:`);
  neg.forEach(word => console.log(word));
  console.log(`The sentiment of the text is ${sent}`);
}
...
function sentiment2(text) {
  const [pos, neg] = determineSent2(text);
  let sent = pos > neg ? 'positive' : 'negative';
  console.log(`There are ${pos.length} positive words in the text:`);
  pos.forEach(word => console.log(word));
  console.log(`There are ${neg.length} negative words in the text:`);
  neg.forEach(word => console.log(word));
